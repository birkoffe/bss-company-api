...
```sh
[I] ➜ grpc_cli call localhost:8082 DescribecompanyV1 "id: 1"
connecting to localhost:8082
Rpc failed with status code 5, error message: company not found
```
...
```sh
[I] ➜ curl -s -X 'POST' \
  'http://localhost:8080/v1/companys' \
...
```sh
$ pgcli "postgresql://docker:docker@localhost:5432/bss_company_api"
```
